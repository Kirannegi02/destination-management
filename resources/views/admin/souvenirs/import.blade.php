...
@section('title', 'Bulk Import Souvenirs')
@section('page-title', 'Bulk Import Souvenirs')
    ...
    <div class="card">
        <div class="card-header" style="align-items: flex-start;">
            ...
            <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                <a href="{{ route('admin.souvenirs.import.sample', ['format' => 'csv']) }}"
                   style="padding: 8px 12px; background: #4299e1; color: white; border-radius: 6px; text-decoration: none; font-size: 14px;">Download CSV Sample</a>
                <a href="{{ route('admin.souvenirs.import.sample', ['format' => 'xls']) }}"
                   style="padding: 8px 12px; background: #667eea; color: white; border-radius: 6px; text-decoration: none; font-size: 14px;">Download Excel Sample</a>
                <a href="{{ route('admin.souvenirs.export', ['format' => 'xls']) }}"
                   style="padding: 8px 12px; background: #48bb78; color: white; border-radius: 6px; text-decoration: none; font-size: 14px;">Export All (Excel)</a>
                <a href="{{ route('admin.souvenirs.export', ['format' => 'csv']) }}"
                   style="padding: 8px 12px; background: #38b2ac; color: white; border-radius: 6px; text-decoration: none; font-size: 14px;">Export All (CSV)</a>
            </div>
        </div>

        <div style="padding: 20px; display: grid; grid-template-columns: 2fr 1fr; gap: 20px; align-items: start;">
            <form action="{{ route('admin.souvenirs.import') }}" method="POST" enctype="multipart/form-data" style="background: #f7fafc; padding: 16px; border-radius: 10px; border: 1px solid #e2e8f0;">
                @csrf
                <div style="display: flex; flex-direction: column; gap: 12px;">
                    ...
                    @enderror
                    <button type="submit" style="padding: 10px 14px; background: #1e3a8a; color: white; border: none; border-radius: 8px; font-size: 15px; cursor: pointer;">Import Souvenirs</button>
                </div>
